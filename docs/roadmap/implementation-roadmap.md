...
- Stage 6D1 - Trusted Image Publication Engine & Contracts: implemented for review; establishes local trusted-publication naming, candidate and authoritative reference construction, digest continuity validation, same-source rerun protection, publication evidence validation, and a Stage 5-compatible `image-reference.json` handoff without registry mutation.
...
- Stage 6D - Trusted OCI Publication & Immutable Digest: in progress; the local engine/contracts slice is implemented, while live GHCR publication is not implemented yet.
