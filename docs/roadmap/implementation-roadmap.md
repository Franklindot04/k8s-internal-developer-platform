...
- Trusted Publication Boundary Hardening: complete / merged; locks private candidate quarantine, attempt-aware candidate identity, rerun identity checks, and authoritative-only handoff boundaries.
- Stage 6D2 - Trusted GHCR Workflow & First Live Publication: implemented for review; adds the trusted protected-main workflow and publication runtime using the Stage 6D1 contracts. Live publication, package creation, private visibility proof, digest pull proof, and public visibility are not yet proven.
- Stage 6D - Trusted OCI Publication & Immutable Digest: in progress; the local engine/contracts slice, boundary hardening, and workflow implementation are complete for review, while live GHCR publication remains unproven until protected merge.
