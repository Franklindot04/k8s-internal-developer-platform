...
- Stage 6C1 - Supply-Chain Evidence Tooling: implemented for local review; establishes one-build Docker archive identity, local runtime proof from that archive, one Syft cataloging operation that emits CycloneDX JSON for portable review and Syft JSON for rich scanner-native inventory evidence, direct Grype scanning of the exact Docker archive, fail-closed vulnerability policy evaluation, and evidence manifest validation without adding PR workflow automation.
- Stage 6C2 - Untrusted PR Verification: implemented for review; wires the Stage 6C1 evidence engine into pull-request CI with read-only permissions, no publication credentials, retained validated evidence, and an always-reporting final gate. Stage 6C remains incomplete until Stage 6C2 is merged, proven on relevant and irrelevant pull-request paths, and audited after merge.
