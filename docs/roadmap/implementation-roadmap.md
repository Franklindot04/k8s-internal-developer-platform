...
- Stage 6D2 - Trusted GHCR Workflow & First Live Publication: in recovery; the first live attempt proved public GHCR package inheritance from this public repository and failed closed before authoritative publication. The recovery workflow uses runner-local quarantine, verified public candidate staging, and an `authoritative-publication` Environment gate before authoritative public publication and anonymous digest proof.
- Stage 6D - Trusted OCI Publication & Immutable Digest: in progress; the local engine/contracts slice, boundary hardening, workflow implementation, and release-boundary recovery are complete for review, while the protected Environment and controlled live retry remain governance prerequisites.
