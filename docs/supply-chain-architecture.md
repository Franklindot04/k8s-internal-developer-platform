...
Stage 6D1 establishes the local trusted-publication engine and contracts without live registry mutation. It locks GHCR repository naming for the representative supply-chain fixture, source-revision and registry-digest validation, private candidate quarantine, attempt-aware non-authoritative candidate references, authoritative source-revision convenience tags, digest continuity checks, same-source rerun safety, publication evidence validation, and the machine-readable `image-reference.json` Stage 5 handoff shape. Candidate evidence uses the planned private quarantine repository `ghcr.io/franklindot04/k8s-internal-developer-platform/supply-chain-fixture-candidates`; authoritative evidence and Stage 5 handoff use only `ghcr.io/franklindot04/k8s-internal-developer-platform/supply-chain-fixture`. Successful publication requires build metadata, candidate registry, verified scan target, authoritative tag, and handoff digests to match the same OCI registry digest even though the candidate and authoritative repositories differ. Candidate-only and policy-blocked states cannot produce a Stage 5 handoff. A same-source rerun may accept an existing matching digest, but it must fail closed if the source-revision tag resolves to a different digest.

Stage 6D2 is implemented for review but not live-proven. It adds the trusted GHCR workflow for protected `main` pushes and a publication runtime that uses the Stage 6D1 contracts instead of reimplementing publication policy in workflow YAML. The workflow pushes unverified candidates only to the private quarantine repository, verifies and scans the exact candidate digest, promotes only a policy-approved digest to the authoritative repository, independently proves final digest equality, and emits a handoff that references only the authoritative repository. The planned authoritative package visibility recommendation is public because Stage 5 does not currently expose complete self-service private-registry authentication, but the authoritative package is created private first and public visibility remains a later explicit governance action. The candidate quarantine package must remain private. Compact trusted publication evidence is retained for 90 days. Failed candidates may remain as non-authoritative private forensic evidence without granting package deletion permission in the initial workflow. This branch does not claim package creation, registry visibility, anonymous pull, or first live publication has occurred.
