...
Stage 6D2 is not implemented yet. It will add the trusted GHCR workflow and first controlled live publication from protected `main`, using the Stage 6D1 engine instead of reimplementing publication policy in workflow YAML. The future workflow must push unverified candidates only to the private quarantine repository, verify and scan the exact candidate digest, promote only a policy-approved digest to the authoritative repository, independently prove final digest equality, and emit a handoff that references only the authoritative repository. The planned authoritative package visibility recommendation is public because Stage 5 does not currently expose complete self-service private-registry authentication, but the authoritative package should be created private first and public visibility remains a later explicit governance action. The candidate quarantine package must remain private. Compact trusted publication evidence should be retained for 90 days. Failed candidates may remain as non-authoritative private forensic evidence without granting package deletion permission in the initial workflow.
