...
Stage 5 introduces a developer-owned service intent contract for the Kubernetes Internal Developer Platform. The contract is now validated and can be compiled into deterministic Stage 4-compatible Helm values and a deterministic Argo CD Application. Stage 5D adds controlled repository generation and read-only drift verification for those artifacts. GitOps registration and any future portal or API remain later work.
...
Stage 5A implemented the versioned contract and validation foundation. Stage 5B added deterministic read-only planning from `PlatformService` intent to golden-path Helm values. Stage 5C added deterministic read-only planning for the matching GitOps Application. Stage 5D adds safe local writes for the generated values/Application pair and a read-only verifier. It does not stage Git changes, commit, push, open pull requests, register Applications with Argo CD, create Kubernetes workloads, add application source code, or implement app-of-apps/ApplicationSet discovery.
...
Generated files are derived from this contract. Helm values and Argo CD Application files must not become competing sources of truth.
...
Write the deterministic generated pair:

```bash
platformctl service generate services/<service>/service.yaml
```

The write-capable command is stricter than `plan`. The source must be a regular non-symlink file named `service.yaml`, located directly at `services/<service>/service.yaml`, and `<service>` must match `metadata.name`. The source and output paths must resolve inside the repository. Developers cannot select arbitrary output directories or filenames.

For `services/payments-api/service.yaml`, generation writes exactly:

```text
services/payments-api/generated/values.yaml
services/payments-api/generated/application.yaml
```

No third persistent generation state file is created. Stage 5D does not add `metadata.yaml`, manifests, generated READMEs, lock metadata, or generation databases.

Both generated files carry the deterministic platform ownership banner:
...
Before overwriting either artifact, the generator validates the complete pair. Existing generated artifacts must be regular files with the exact ownership marker. Unmanaged files, symlinks, directories, special files, and unexpected persistent entries under `generated/` are refused before mutation. There is no `--force` option in Stage 5D.

Repository static validation accepts both source-only services and canonical platform-owned generated pairs. It rejects partial pairs, missing source files, unowned generated artifacts, symlink or special-file hazards, and unexpected persistent entries. Static validation does not compare generated bytes with compiler output; `platformctl service verify` owns deterministic drift detection.

Generation computes both artifacts in memory before writing. It stages temporary files beside the final outputs, flushes them, replaces each artifact atomically at the file level, attempts rollback to the previous exact bytes after ordinary replacement failures, cleans temporary files, and verifies final bytes before reporting success. This protects ordinary execution failures, but it is not a single crash-atomic POSIX transaction across two independent files. If a process or host terminates between replacements, `verify` detects the resulting state.

Generation also uses a POSIX advisory lock on the service directory so simultaneous generators for the same service do not interleave writes. The lock does not create committed repository metadata.

Repeated generation is idempotent. Current owned artifacts are reported as unchanged. Owned generated drift is repaired by replacing the drifted artifact with deterministic compiler output.

Generation does not run `git add`, commit, push, open pull requests, call GitHub, call Kubernetes, call Argo CD, create AppProjects, register Applications, or trigger ApplicationSet/app-of-apps behavior.

## Drift Verification

Verify generated state without writing:

```bash
platformctl service verify services/<service>/service.yaml
```

`verify` uses the compiler output as authority, not source-file byte equality. A `PlatformService` edit that does not affect generated output does not create artificial drift. The verifier exits successfully only when both generated files are owned and byte-exact. Other states, including `missing`, `drifted`, `unowned`, and `unexpected-entry`, fail without modifying artifact bytes, directory contents, source files, ownership markers, or repository state.

## GitOps Relationship

Stage 5C derives an Argo CD Application artifact from the handwritten service contract. Stage 5D writes it locally with the matching values artifact:
...
The generated Application is a GitOps-ready artifact, not automatically discovered by an app-of-apps mechanism. Stage 5D does not register that Application with Argo CD, modify the Stage 3 bootstrap Application, create an ApplicationSet, or add app-of-apps discovery.
