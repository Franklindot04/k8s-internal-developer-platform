...
if File.file?(trusted_publication_runtime)
  ...
  fail_with('trusted publication runtime must classify authoritative package metadata explicitly') unless runtime.include?('classify_authoritative_package_response')
  fail_with('trusted publication runtime must validate source repository token before package absence') unless runtime.include?('validate_source_repository_token')
  fail_with('trusted publication runtime must corroborate exact package 404 with namespace listing') unless runtime.include?('corroborate_authoritative_package_absence')
  fail_with('trusted publication runtime must classify package namespace listing explicitly') unless runtime.include?('classify_package_namespace_response')
  fail_with('trusted publication runtime must classify authoritative source tags explicitly') unless runtime.include?('classify_authoritative_versions_response')
  fail_with('trusted publication runtime must use GitHub Packages API for authoritative precheck') unless runtime.include?('GITHUB_PACKAGES_API_ROOT')
