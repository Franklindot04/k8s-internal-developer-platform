...
if File.file?(trusted_publication_runtime)
  ...
  fail_with('trusted publication runtime must route recognized public states through semantic state') unless runtime.include?('AUTHORITATIVE_STATE_CLASSIFICATION')
  fail_with('trusted publication runtime must not encode public-unobservable as fatal status 5') if runtime.include?('return 5')
  fail_with('trusted publication runtime must route public-unobservable into local quarantine') unless runtime.include?('next_stage=local-quarantine')
  fail_with('trusted publication runtime must derive repository root from script location') unless runtime.include?('BASH_SOURCE[0]') && runtime.include?('REPO_ROOT')
  fail_with('trusted publication runtime must anchor vulnerability evaluator to repository root') unless runtime.include?('repo_path scripts/supply-chain/evaluate-vulnerabilities.rb')
  fail_with('trusted publication runtime must not invoke vulnerability evaluator through cwd-relative path') if runtime.include?('ruby scripts/supply-chain/evaluate-vulnerabilities.rb')
  ...
  fail_with('trusted publication runtime must validate policy-result artifacts structurally') unless runtime.include?('policy_result_decision()') && runtime.include?('File.file?(path)') && runtime.include?('File.zero?(path)') && runtime.include?('JSON.parse(File.read(path))')
  fail_with('trusted publication runtime must anchor policy-result parser to repository root') unless runtime.include?('repo_path scripts/supply-chain/publication.rb')
  fail_with('trusted publication runtime must accept only PASS or FAIL policy decisions') unless runtime.include?('SupplyChainPublication::PASS') && runtime.include?('SupplyChainPublication::FAIL')
  fail_with('trusted publication runtime must clear stale policy results before evaluation') unless runtime.include?('rm -f "$policy_result"')
  local_policy_handoff = 'LOCAL_POLICY_DECISION="$(policy_result_decision "$WORK_DIR/local-policy-result.json")"'
  local_policy_pass = '[ "$LOCAL_POLICY_DECISION" = "PASS" ] || fail "local vulnerability policy did not pass"'
  fail_with('trusted publication runtime must consume local policy result in the parent shell') unless runtime.include?(local_policy_handoff)
  fail_with('trusted publication runtime must not assign local policy through the evidence-generation subshell') if runtime.include?('LOCAL_POLICY_DECISION="$decision"')
  ...
  fail_with('trusted publication runtime must perform authenticated source check after local policy') unless runtime.include?('classify_authoritative_manifest_state "authenticated" "authenticated-source-check"') && runtime.index(local_policy_pass) < runtime.index('classify_authoritative_manifest_state "authenticated" "authenticated-source-check"')
