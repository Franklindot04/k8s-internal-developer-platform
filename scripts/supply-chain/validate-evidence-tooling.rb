...
if File.file?(trusted_publication_runtime)
  ...
  fail_with('trusted publication runtime must fail closed on authoritative digest collision') unless runtime.include?('PRE_PROMOTION_EXISTS_DIFFERENT_DIGEST') && runtime.include?('authoritative source tag already exists with different digest')
